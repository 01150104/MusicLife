...
export default function HistoryQuizScreen({ onNavigate }) {
  const navigate = onNavigate || (() => {});

  const questions = useMemo(() => {
    const pool = buildPool(historyQuizData);
  ...
  const handleNext = () => {
    ...
    if (current + 1 >= total) {
      ...
      try {
        const prev = parseInt(localStorage.getItem("quiz_history_best") || "0", 10);
        if (correct > prev) localStorage.setItem("quiz_history_best", String(correct));
      } catch {}
    } else {
  ...
  return (
    <>
      ...
      <div className="qz-page">
        <div className="qz-back" onClick={() => navigate("history")}>
        ...
        {!done ? (
          <>
            <div className="qz-header">
              <div className="qz-title">역사 퀴즈</div>
          ...
          <div className="qz-result">
            ...
            <div className="qz-result-btns">
              ...
              <button className="qz-btn-home" onClick={() => navigate("history")}>역사로 돌아가기</button>
            </div>
          </div>
        )}
      </div>
